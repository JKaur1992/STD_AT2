...
population_subset <- PopulationXl %>%
  ...
  select(LGA, Council_vs_Area ,Year,`Person_Population_Number_20-24_years..55`, `Person_Population_Number_25-29_years..56`, `Person_Population_Number_30-34_years..57`, `Person_Population_Number_35-39_years..58`, `Person_Population_Number_40-44_years..59`, `Person_Population_Number_45-49_years..60` ,Person_Population_Number_Total..69 ,Population_density)
...
Pop_Crime_Merge_DT <- filter(Pop_Crime_Merge_DT, ( LGA == 'Sydney' | LGA == 'Randwick' | LGA == 'Canterbury-Bankstown' | LGA == 'Parramatta' | LGA == 'North Sydney' | LGA == 'Inner West' | LGA == 'Hornsby' | LGA == 'Strathfield' | LGA == 'Burwood' | LGA == 'Mosman')) #  | LGA == 'Bayside'))| LGA == 'Central Darling'  ##Bayside has (C) and (A) creating duplicates 
...
Pop_Crime_Merge_DT <- Pop_Crime_Merge_DT %>%
  group_by(LGA, Year) %>%
  mutate(PopDrinkingAge = sum(`Person_Population_Number_20-24_years..55`,`Person_Population_Number_25-29_years..56`, `Person_Population_Number_30-34_years..57`, `Person_Population_Number_35-39_years..58`, `Person_Population_Number_40-44_years..59`, `Person_Population_Number_45-49_years..60`))
...
Pop_Crime_Merge_DT <- Pop_Crime_Merge_DT %>%
  mutate(CrimePerPerson = violence_count/ Person_Population_Number_Total..69)

#Violenece per ADULT in the drinking Age
Pop_Crime_Merge_DT <- Pop_Crime_Merge_DT %>%
  mutate(CrimePerPerson_20_50 = violence_count/ PopDrinkingAge)
...
ggplot(data = Pop_Crime_Merge_DT) + 
  geom_line(mapping = aes(x = Year, y = CrimePerPerson , color=LGA), show.legend = FALSE)+
...
#Total crimes divided by person between 20 and 50 Years 
ggplot(data = Pop_Crime_Merge_DT) + 
  geom_line(mapping = aes(x = Year, y = CrimePerPerson_20_50 , color=LGA), show.legend = FALSE)+
  xlab("Year") + ylab("Crime Rate") +
  facet_wrap(.~ LGA, scales = "free")+
  geom_vline(xintercept = (2014), linetype="dotted", color = "black", size=1.5)+
  labs(title="Crime Rate", subtitle = "Population between 20-50 Years")
